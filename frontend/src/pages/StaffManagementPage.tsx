      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-white/60">Manage nurse accounts and portal access</p>
          </div>
        ...
        {/* Messages */}
        {msg && (
          <div className="mb-5 flex items-center gap-2 bg-green-400/15 border border-green-400/30 rounded-xl p-4 text-sm text-green-200">
            <CheckCircle className="w-4 h-4" /> {msg}
          </div>
        )}
        {error && (
          <div className="mb-5 flex items-center gap-2 bg-red-400/15 border border-red-400/30 rounded-xl p-4 text-sm text-red-200">
        ...
        {showForm && (
          <form onSubmit={handleCreate} className="card mb-6">
            <h2 className="font-bold text-white mb-4">Create Nurse Account</h2>
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2">
                ...
                <p className="text-xs text-white/55 mt-1">Share this password securely with the nurse. They can change it after first login.</p>
              </div>
            </div>
                ...
                <div key={s.id} className="flex items-center justify-between p-4 glass-pill rounded-xl">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 glass rounded-full flex items-center justify-center">
                      <span className="text-white font-bold text-sm">{s.first_name[0]}{s.last_name[0]}</span>
                    </div>
                    <div>
                      <p className="font-semibold text-white text-sm">{s.first_name} {s.last_name}</p>
                      <p className="text-xs text-white/55">{s.email}</p>
                      {s.phone && <p className="text-xs text-white/45">{s.phone}</p>}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    {s.is_active && (
                      <button
                        ...
                        className="text-xs text-red-300 hover:text-white font-medium px-3 py-1 border border-red-400/30 rounded-lg hover:bg-red-400/15 transition-colors"
                      >
                        Deactivate
                      </button>
                    )}
