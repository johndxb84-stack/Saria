    ...
    <div className="min-h-screen text-gray-900">
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          <button onClick={() => navigate(dashboardPath)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors text-gray-900">
        ...
        {user?.role === 'patient' && (
          <form onSubmit={handleProfileSave} className="card mb-6">
            ...
            {saveMsg && (
              <div className="mb-4 flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-sm text-emerald-600">
                <CheckCircle className="w-4 h-4 text-emerald-600" /> {saveMsg}
              </div>
            )}
            {saveError && (
              <div className="mb-4 flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl p-3 text-sm text-red-600">
                <AlertCircle className="w-4 h-4 text-red-500" /> {saveError}
              </div>
            )}
        ...
        <form onSubmit={handlePasswordChange} className="card">
          ...
          {pwMsg && (
            <div className="mb-4 flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-sm text-emerald-600">
              <CheckCircle className="w-4 h-4 text-emerald-600" /> {pwMsg}
            </div>
          )}
          {pwError && (
            <div className="mb-4 flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl p-3 text-sm text-red-600">
              <AlertCircle className="w-4 h-4 text-red-500" /> {pwError}
            </div>
          )}
