...
const TYPE_CONFIG: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  consultation: { label: 'Consultation', color: 'badge-blue', icon: <FileText className="w-4 h-4" /> },
  blood_test: { label: 'Blood Test', color: 'badge-red', icon: <FlaskConical className="w-4 h-4" /> },
  xray: { label: 'X-Ray', color: 'badge-purple', icon: <Image className="w-4 h-4" /> },
  prescription: { label: 'Prescription', color: 'badge-green', icon: <Pill className="w-4 h-4" /> },
  vaccination: { label: 'Vaccination', color: 'badge-yellow', icon: <Shield className="w-4 h-4" /> },
  allergy: { label: 'Allergy', color: 'badge-yellow', icon: <AlertCircle className="w-4 h-4" /> },
  surgery: { label: 'Surgery', color: 'badge-red', icon: <Heart className="w-4 h-4" /> },
  note: { label: 'Note', color: 'badge-gray', icon: <FileText className="w-4 h-4" /> },
  other: { label: 'Other', color: 'badge-gray', icon: <FileText className="w-4 h-4" /> },
};
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex gap-1 mb-6 bg-gray-100 p-1 rounded-xl w-fit">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'records' ? 'bg-white shadow text-gray-900' : 'text-gray-600 hover:text-gray-900'}`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'files' ? 'bg-white shadow text-gray-900' : 'text-gray-600 hover:text-gray-900'}`}
          >
            <span className="flex items-center gap-2">
                ...
                filtered.map(record => {
                  ...
                  return (
                    <div key={record.id} className="card p-0 overflow-hidden hover:shadow-md transition-shadow">
                      ...
                      >
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${cfg.color.includes('blue') ? 'bg-blue-50 text-blue-600' : cfg.color.includes('red') ? 'bg-red-50 text-red-600' : cfg.color.includes('purple') ? 'bg-purple-50 text-purple-600' : cfg.color.includes('green') ? 'bg-green-50 text-green-600' : 'bg-gray-50 text-gray-600'}`}>
                          {cfg.icon}
                        </div>
                                ...
                                <table className="w-full text-sm border-collapse">
                                  <thead>
                                    <tr className="bg-gray-50 text-left">
                                      <th className="px-3 py-2 text-xs font-semibold text-gray-600 rounded-tl-lg">Test</th>
                                      <th className="px-3 py-2 text-xs font-semibold text-gray-600">Value</th>
                                      ...
                                      <th className="px-3 py-2 text-xs font-semibold text-gray-600 rounded-tr-lg">Status</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                            ...
                            <div>
                              ...
                              <div className="space-y-2">
                                {record.prescriptions.map(rx => (
                                  <div key={rx.id} className="bg-green-50 rounded-lg p-3 text-sm">
                                ...
                                {record.files.map(f => (
                                  <div key={f.id} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2">
                                    ...
                                    <button
                                      ...
                                      className="text-primary-600 hover:text-primary-700 flex items-center gap-1 text-xs font-medium ml-2"
                                    >
                ...
                {files.map(f => (
                  <div key={f.id} className="card hover:shadow-md transition-shadow">
                    <div className="flex items-start gap-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${f.file_category === 'xray' || f.file_category === 'scan' ? 'bg-purple-50 text-purple-600' : 'bg-blue-50 text-blue-600'}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {f.record_title && <p className="text-xs text-primary-600 mt-1">📋 {f.record_title}</p>}
                    ...
                    <button
                      onClick={() => handleDownload(f.id, f.original_name)}
                      className="mt-3 w-full flex items-center justify-center gap-2 py-2 text-sm text-primary-600 hover:text-primary-700 border border-primary-200 hover:border-primary-300 rounded-lg transition-colors"
                    >
